...
class Experience(models.Model):
    ...
    def clean(self):
        if (self.is_current and self.to_date is not None) or self.from_date is None or (self.to_date is None and not self.is_current) or (self.to_date == self.from_date):
        ...
        if self.to_date is not None and (self.to_date < self.from_date and self.to_date < timezone.now().date()):
            raise ValidationError('Invalid date range, please check your dates. got to_date: {} and from_date: {}'.format(self.to_date, self.from_date))
